...
def train_internal(data_dir, config, device):
    train_dataset = MNISTDigitDataset(
        data_dir, [0], class_size=10, partition="labeled"
    )

    validation_dataset = MNISTDigitDataset(
        data_dir, [0], class_size=10, partition="valid"
    )

    backbone = BACKBONE_MODELS[config["Backbone"]["Model"]](num_classes=config["Backbone"]["Hidden-Size"])
    prediction_head = MLP(config["Backbone"]["Hidden-Size"], config["Projection-Head"]["Hidden-Size"], 10,
...
def train(experiment, split, train_size, unlabeled, overlap, config, device, backbone=None, prediction_head=None, model=None):
    data_dir = f"{THIS_DIR}/../data/{experiment}/split::{split}/train-size::{train_size}/unlabeled::{unlabeled}/overlap::{overlap}"
    ...
    print(f"Saving trained models for {experiment}/split::{split}/train-size::{train_size}/unlabeled::{unlabeled}/overlap::{overlap}...")
...
def hyperparameter_search(experiment, split, train_size, unlabeled, overlap, config, device):
    best_validation_accuracy = 0.0
    best_config = copy.deepcopy(config)

    data_dir = f"{THIS_DIR}/../data/{experiment}/split::{split}/train-size::{train_size}/unlabeled::{unlabeled}/overlap::{overlap}"

    print(f"Hyperparameter search for {experiment}/split::{split}/train-size::{train_size}/unlabeled::{unlabeled}/overlap::{overlap}...")
...
def main():
    ...
    for experiment in ["mnist-1", "mnist-2", "mnist-4"]:
                ...
                for overlap in ["0.00"]:
                    ...
                    for split in ["0", "1", "2", "3", "4"]:
                        ...
                        if os.path.exists(f"{experiment_dir}/saved-networks/{TRAINED_MODEL_NAME}"):
                            print(f"Found trained models for {experiment}/split::{split}/train-size::{train_size}/unlabeled::{unlabeled}/overlap::{overlap}. Skipping...")
                            continue

                        print(f"Training {experiment}/split::{split}/train-size::{train_size}/unlabeled::{unlabeled}/overlap::{overlap}...")

                        if split == "0":
                            best_config = hyperparameter_search(
                                experiment, split, train_size, unlabeled, overlap, copy.deepcopy(CONFIG), device
                            )

                            backbone, prediction_head, model = train(
                                experiment, split, train_size, unlabeled, overlap, best_config, device
                            )
                        else:
                            train(
                                experiment, split, train_size, unlabeled, overlap, best_config, device,
