...
DATA_DIR_FORMAT = os.path.join(BASE_DATA_DIR, '{}', '{}', '{}', '{}', 'overlap::{}')
...
def main():
    ...
    for experiment in sorted(os.listdir(RESULTS_DIR)):
        ...
        for model in sorted(os.listdir(os.path.join(RESULTS_DIR, experiment))):
            ...
            dataset = experiment_utils.get_dataset_from_model(model)
            ...
            for log_path in log_paths:
                ...
                if len(results[experiment][model]['rows']) == 0:
                    ...
                    results[experiment][model]['header'].append('IMAGEDIGIT_ADDITION_CONSISTENCY')
                    results[experiment][model]['header'].append('NEURALCLASSIFIER_ADDITION_CONSISTENCY')
                ...
                print(os.path.join(os.path.dirname(log_path), INFERRED_PREDICATES_DIR_NAME, "IMAGEDIGIT.txt"))
                results[experiment][model]['rows'][-1].append(
                    experiment_utils.get_addition_consistency(
                        os.path.join(os.path.dirname(log_path), INFERRED_PREDICATES_DIR_NAME, "IMAGEDIGIT.txt"),
                        os.path.join(DATA_DIR_FORMAT.format(dataset, *parts, "0.00"), "image-sum-truth-test.txt")
                    )
                )

                print(os.path.join(os.path.dirname(log_path), INFERRED_PREDICATES_DIR_NAME, "NEURALCLASSIFIER.txt"))
                results[experiment][model]['rows'][-1].append(
                    experiment_utils.get_addition_consistency(
