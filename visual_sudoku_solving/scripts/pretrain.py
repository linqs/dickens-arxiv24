...
def main():
    ...
    for experiment in ["mnist-9x9"]:
        for split in ["0", "1", "2", "3", "4"]:
            backbone = None
            pretrained_projection_head = None
            model = None
            for train_size in ["0020"]:
                for num_clues in ["30"]:
                    for unlabeled in ["0.00", "0.50", "0.90", "0.95", "1.00"]:
                        experiment_dir = f"{THIS_DIR}/../data/{experiment}/split::{split}/train-size::{train_size}/num-clues::{num_clues}/unlabeled::{unlabeled}"
                        if os.path.exists(f"{experiment_dir}/saved-networks/{TRAINED_MODEL_NAME}"):
                            print(f"Found pretrained models for {experiment}/split::{split}/train-size::{train_size}/num-clues::{num_clues}/unlabeled::{unlabeled}. Skipping...")
                            continue
                        ...
                        if model is None:
                            train_dataset = MNISTDigitDataset(
                                experiment_dir, [0, 1, 2], class_size=9, partition="train", id_labels=True
                            )
                            ...
                            model = MNISTClassifier(backbone, MLP(128, 64, 9, 2).to(device), device=device)
